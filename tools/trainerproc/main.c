...
static bool parse_trainer(struct Parser *p, const struct Parsed *parsed, struct Trainer *trainer)
{
    ...
    *trainer = (struct Trainer) {};
    ...
    while (parse_attribute(p, &key, &value))
    {
        ...
        {
            any_error = !set_show_parse_error(p, key.location, "expected one of 'Name', 'Class', 'Pic', 'Back Pic', 'Gender', 'Music', 'Items', 'Battle Type', 'Party Size', 'Pool Rules', 'Pool Pick Functions', 'Pool Prune' or 'AI'");
        }
    }
    ...
    for (int i = 0; i < PARTY_SIZE; i++)
    {
        ...
        while (parse_attribute(p, &key, &value))
        {
            ...
            {
                any_error = !set_show_parse_error(p, key.location, "expected one of 'EVs', 'IVs', 'Ability', 'Level', 'Ball', 'Happiness', 'Nature', or 'Shiny'");
            }
        }
...
static void fprint_trainers(const char *output_path, FILE *f, struct Parsed *parsed)
{
    ...
    for (int i = 0; i < parsed->trainers_n; i++)
    {
        ...
        {
            // This doesn't really do much for now...
            // fprintf(f, "#line %d\n", trainer->gender_line);
            // fprintf(f, "        .gender = ");
        ...
        if (!is_empty_string(trainer->encounter_music))
        {
            ...
            if (!is_empty_string(trainer->gender) && strcasecmp(trainer->gender.string, "female"))
            {
                // Annoying edge case :V
                fprint_constant(f, " | F_TRAINER", trainer->gender);
            }
            fprintf(f, ",\n");
        }
        ...
        if (trainer->ai_flags_n > 0)
        {
            ...
            for (int i = 0; i < trainer->ai_flags_n; i++)
            {
                if (i > 0)
                    fprintf(f, " | ");
                fprint_constant(f, "AI_SCRIPT", trainer->ai_flags[i]);
            }
            fprintf(f, ",\n");
        }
