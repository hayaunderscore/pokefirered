...
static const u8 sText_Desc_TextSpeed[]          = _("Choose one of the four text\ndisplay speeds.");
static const u8 sText_Desc_BattleScene_On[]     = _("Show the POKéMON battle\nanimations.");
static const u8 sText_Desc_BattleScene_Off[]    = _("Skip the POKéMON battle\nanimations.");
static const u8 sText_Desc_BattleStyle_Shift[]  = _("Get the option to switch your\nPOKéMON after the enemies faints.");
static const u8 sText_Desc_BattleStyle_Set[]    = _("No free switch after fainting\nthe enemies POKéMON.");
static const u8 sText_Desc_SoundMono[]          = _("Sound is the same in all speakers.\nRecommended for original hardware.");
static const u8 sText_Desc_SoundStereo[]        = _("Play the left and right channel\nseperately. Great with headphones.");
static const u8 sText_Desc_ButtonMode_LR[]      = _("On some screens the L and R\nbuttons act as left and right.");
...
static void DrawTopBarText(void)
{
    ...
    AddTextPrinterParameterized3(WIN_TOPBAR, FONT_SMALL, x, 0, color, 0, gText_PickSwitchCancel);
...
static const u16 sOptionMenuItemCounts[MENUITEM_MAIN_COUNT] = {3, 2, 2, 2, 2, 2, 10, 0};

void CB2_SetOptionsPlusMenuOptions(void)
{
	u8 i;
	u16 textSpeed;

	sOptions = AllocZeroed(sizeof(struct OptionMenu));
    ...
    // for (i = 0; i < MENUITEM_MAIN_COUNT - 1; i++)
    // {
    //     if (sOptions->sel[i] > (sOptionMenuItemCounts[i]) - 1)
    //         sOptions->sel[i] = 0;
    // }
...
void CB2_InitOptionPlusMenu(void)
{
    ...
    case 2:
        ...
        break;
    case 3:
        LoadBgTiles(1, GetUserWindowGraphics(sOptions->sel[MENUITEM_MAIN_FRAMETYPE])->tiles, 0x120, 0x1A2);
        gMain.state++;
        break;
    case 4:
        LoadPalette(sOptionMenuBg_Pal, 0, sizeof(sOptionMenuBg_Pal));
        LoadPalette(GetUserWindowGraphics(sOptions->sel[MENUITEM_MAIN_FRAMETYPE])->palette, 0x70, 0x20);
        gMain.state++;
        break;
    case 5:
...
static int ProcessInput_FrameType(int selection)
{
    if (JOY_NEW(DPAD_RIGHT))
    {
        ...
        LoadBgTiles(1, GetUserWindowGraphics(selection)->tiles, 0x120, 0x1A2);
        LoadPalette(GetUserWindowGraphics(selection)->palette, 0x70, 0x20);
    }
    if (JOY_NEW(DPAD_LEFT))
    {
        if (selection != 0)
...
static void DrawChoices_ButtonMode(int selection, int y)
{
    ...
    DrawOptionMenuChoice(gText_ButtonTypeLR, 104, y, styles[0], active);
    DrawOptionMenuChoice(gText_ButtonTypeLEqualsA, GetStringRightAlignXOffset(1, gText_ButtonTypeLEqualsA, 198), y, styles[1], active);
}

static const u8 sText_QuestLogOn[]     = _("ON");
static const u8 sText_QuestLogOff[]    = _("OFF");
static void DrawChoices_QuestLog(int selection, int y)
{
    bool8 active = CheckConditions(MENUITEM_MAIN_QUESTLOG);
    u8 styles[3] = {0};
    styles[selection] = 1;

    DrawOptionMenuChoice(sText_QuestLogOn, 104, y, styles[0], active);
    DrawOptionMenuChoice(sText_QuestLogOff, GetStringRightAlignXOffset(1, sText_QuestLogOff, 198), y, styles[1], active);
}
...
static void DrawChoices_FrameType(int selection, int y)
{
    ...
    ConvertIntToDecimalStringN(text, n, 1, 2);
