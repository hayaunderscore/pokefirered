# Usage: python3 migrate_incgfx.py FILE...
# The files will be updated in-place.
#
# To migrate all files you could try something like:
...
def try_convert_incbin(size, c_source, pos, targets):
    ...
    if rule := lookup_rule(target, targets):
        (target_ext, source, arguments) = rule
        if arguments:
            return f'INCGFX_U{size}("{source}", "{target_ext}", "{arguments}")' + c_source[pos:]
        else:
            return f'INCGFX_U{size}("{source}", "{target_ext}")' + c_source[pos:]
    else:
        return None
...
def convert_incbins(path, targets):
    ...
    pattern = re.compile(r'INCBIN_U(8|16|32)')
    pos = 0
    while True:
